...
レポートは作業日誌や会話の続きではなく、確定したシステムと検証結果を新規参加者へ説明する成果物とする。

- 会話、調査、修正、レビュー対応の順に節を継ぎ足さない。
- 最終設計を最初から計画していたものとして、結論、契約、処理フロー、分岐、実装、検証の順に全面的に組み直す。
- 「前回」「追加対応」「この比較を読む鍵」のように、経緯を知らなければ意味が決まらない文を残さない。
- 途中案や訂正は削除する。最終設計の理由として必要な判断だけを、採用しなかった案と理由へ移す。
- 更新後はレポート単体を読み、対象システムを初めて見る読者が入力から出力まで追えるか確認する。
