...
3. `templates/report-outline.md` でレビュー内容を組み立て、`writing/html` でHTMLへ実装する。00では、対象の文脈、変更範囲マップ、変更要約、レビュー導線、判断事項、検証結果の順に示す。
4. 読者向けの語を先に使い、内部名は対応付けてから出す。変更領域ごとに、全体での役割、何を変えたか、変更目的、変更方法、テストまたは実測、保証範囲を結ぶ。H/P/Uは導線を短くする場合だけ使う。
...
10. 詳細は必要なものだけを折りたたむ。実装差分のレビューが主目的なので、図は文章とコードだけでは責務や処理経路を追えない場合に限る。
