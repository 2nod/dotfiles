...
Codex でこの skill を使い、task の一覧を取得できる場合は、worktree path と task の `cwd` を照合し、関連する task とその状態も確認する。
...
4. Codex でこの skill を使う場合は、利用可能な thread tool で task の一覧を取得し、各 worktree の exact path と task の `cwd` を照合する。
   - 一致する task は、title を改変せず、実行状態と pinned / archived の別を記録する。
   - `idle` や `notLoaded` を完了済みと解釈しない。
   - 削除済みの worktree path を参照し続ける task は、worktree とは別の archive 候補として示す。ユーザーの許可なしに archive しない。
   - task 情報を取得できない、または一致がない場合は推測せず「該当 task を確認できず」とする。
5. worktree を分類する。
   ...
   - 公開済み Site のローカル原本だけが ignored / untracked artifact として残っている場合、ユーザーから保存方針の明示がなくても「差分を捨てるなら消せる」削除候補にする。
     Site が worktree から独立して公開済みであることを確認し、削除候補の報告ではローカル原本と再編集可能性が失われることを伝える。実際の削除には手順6の明示確認が必要である。
     この例外は未 push commit、通常の source/test、`docs/plans`、または Site 以外の未保存成果物には適用しない。
6. 既存 worktree を消す前、または local change を捨てる前に、必ずユーザーの明示確認を取る。
7. clean な worktree は `git -C <primary-repo> worktree remove <path>` で消す。
   `--force` は、捨てる差分を列挙し、ユーザーが同意した場合だけ使う。
8. worktree を削除した後で、local branch と remote branch を別々に分類する。
...
9. 削除後は `git worktree list --porcelain`、`git worktree prune --dry-run --verbose`、branch、path の存在確認で取りこぼしを確認する。
...
- Codex で複数 worktree を一覧にするときは、関連する Codex task の title、実行状態、pinned / archived の別も同じ行または直後に含める。他の agent ではこの項目を必須にしない。
