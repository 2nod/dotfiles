...
skill 本体は共通。`agent-skills-nix` が `.agents/skills` を bundle し、各 agent の runtime path へ deploy する。
...
`.agents/installed-skills` は upstream 保管庫。runtime では personal wrapper 経由で使う。
...
件数は `.agents/skills` の authored skill 数と揃うはず。`.agents/installed-skills` だけ追加されても deploy 対象にはならない。
