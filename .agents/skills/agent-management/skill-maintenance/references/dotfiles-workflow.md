...
ローカル向けに trigger や guardrail を変えたい場合は installed skill を編集しない。
現行では `.agents/skills` と `.agents/installed-skills` の同じ公開 path は使わない。
agent 全体に効かせる制約は agent-specific instruction に置き、skill として発火させたい制約は別名の自作 wrapper skill にする。

同じ公開 path で installed skill を shadow する方式は、`agent-management/skill-governance` で方針を決め、trigger check と deploy の優先順位を実装してから使う。
...
現行では自作 skill と installed skill の公開 path を重複させない。
