...
1. personal skill として `.agents/skills/<category>/<skill-name>/SKILL.md` を作る。
...
runtime で使う場合は必ず同じ public path の personal wrapper skill を `.agents/skills` に作り、routing は wrapper 優先にする。
...
ローカル向けに trigger、guardrail、template、手順を変えたい場合は installed skill を編集せず、`.agents/skills/<category>/<skill-name>` に personal wrapper skill を作る。
personal wrapper の public path は installed skill と同じにし、agent から見える entrypoint を wrapper に一本化する。

## 即時公開

Home Manager は `.agents/skills` を先に publish し、その後 `.agents/installed-skills` を publish する。
同じ public path がある場合は `.agents/skills` の personal wrapper を優先し、installed skill 側は skip する。
すぐ使いたい場合は Codex / Cursor Agent / Claude Code などの skills directory に symlink を張る。
外部 skill directory を直接読める agent でも、同じ public path に personal wrapper がある場合は wrapper を entrypoint にする。
...
installed skill を runtime で使う場合、同じ public path の personal wrapper が `.agents/skills` にあることも `.agents/bin/check-skill-triggers` で確認する。
