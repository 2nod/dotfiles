...
function fish_right_prompt
    if test $CMD_DURATION
        if test $CMD_DURATION -gt $NOTIFY_ON_COMMAND_DURATION
            ...
            type -q terminal-notifier && terminal-notifier -message $MSG -execute "(command -v wezterm) cli activate-pane --pane-id $WEZTERM_PANE" -activate com.github.wez.wezterm \  && return
        end
    end
end
